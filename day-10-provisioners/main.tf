...
resource "aws_instance" "server" {
  ...
  provisioner "local-exec" {
    command = "echo 'Server created successfully!' > result.txt"
  }
}

# Re-run script each time (forces rerun)
resource "null_resource" "run_script" {
  ...
  triggers = {
    always_run = timestamp() # ✅ Forces rerun each apply
  }
}
